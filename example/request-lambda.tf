...
resource "aws_lambda_permission" "apigwR" {
  ...
  source_arn = "arn:aws:execute-api:ap-northeast-2:${secrets.AWS_USER_NUM}:${aws_api_gateway_rest_api.jkApi.id}/*/${aws_api_gateway_method.proxy.http_method}${aws_api_gateway_resource.proxy.path}"
}